...
func _run() -> void:
	...
	# poll_data() will emit `data_received`/`process_exited` for backward compatibility,
	...
	ok = ok and T.require_eq(self, int(_pty.open(80, 24, "cmd.exe /C echo hello")), 0, "open() should return OK")
	if not ok:
		return
