...
func _test_basic() -> bool:
	...
	tp.add_async_hyperlink_filter(TestFilter.new(true))

	var link := TestFilter.format_link("hello")
...
func _test_erase() -> bool:
	...
	var link := TestFilter.format_link("simple")
	session.process(link)
...
func _test_osc_link() -> bool:
	...
	session.terminal.set_url_hyperlink_filter(TestSyncFilter.new())

	var hyperlink_style = _HyperlinkStyleScript.make()
	var normal := session.get_current_style()

	session.process("\u001B]8;;" + TestFilter.format_link("foo") + "\u001B\\Foo link\u001B]8;;\u001B\\ Some text 1")
	...
	session.process("\r\n")
	session.process("\u001B]8;;" + TestFilter.format_link("bar") + "\u0007Bar link\u001B]8;;\u0007 Some text 2")
...
func _test_link_after_horizontal_resize() -> bool:
	var session := TestSession.new(100, 5)
	var test_filter := TestFilter.new(false)
	session.get_text_processing().add_async_hyperlink_filter(test_filter)

	session.process("1_2_3_4_5_6_7_8_9 1_2_3_4_5_6_7_8_9 " + TestFilter.format_link("foo"))
...
func _test_link_after_horizontal_resize_and_history() -> bool:
	...
	session.process("1_2_3_4_5_6_7_8_9 " + TestFilter.format_link("foo") + " a-b-c-d-e-f-g-h-i-j-k-l-m-n-o")
