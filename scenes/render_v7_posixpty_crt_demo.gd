...
@export var shell_command: String = "/bin/zsh -l"

@onready var crt_terminal = $CrtTerminal
@onready var terminal_control: Control = $CrtTerminal.get_terminal_control()
...
func _process(_delta: float) -> void:
	...
	info.text = "Render v7 PosixPTY CRT demo | FPS:%d | PTY:%s | Font:%s@%d" % [
		fps,
...
func _try_start_posix_pty() -> void:
	if OS.get_name() == "Windows":
		status.text = "PosixPTY: non-Windows only"
		return
	if not ClassDB.class_exists("PosixPTY"):
		status.text = "PosixPTY: 未启用扩展（请在 Project Settings → GDExtension 添加 res://addons/jediterm/native/conpty.gdextension，并先本地构建动态库）"
		return

	_pty = ClassDB.instantiate("PosixPTY")
	if _pty == null:
		status.text = "PosixPTY: instantiate failed"
		return
